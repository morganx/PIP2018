---
title: "MasterPip - the unfiltered version"
...
Taxonomy_unfiltered<-rbind(E_coli_abundance_AtBirth,E_coli_abundance_3_month,E_coli_abundance_12_month,E_coli_abundance_24_month)


```

Plots: Regarding the relationships between E. coli abundance & age, E. coli abundance & time at room temperature, and time of storage of samples (Sup 1)
This figure does not change in unfiltered version
...
Norm_unfiltered_taxonomy_abundance_select<-Norm_log10_abundance[,grep("g__",colnames(Norm_log10_abundance))]
...
Norm_unfiltered_taxonomy_g<-Norm_unfiltered_taxonomy_abundance_select[,setdiff(colnames(Norm_unfiltered_taxonomy_abundance_select),NZGL_taxonomy_select_t_col)]
...
Norm_unfiltered_taxonomy_g$time<-Taxonomy_unfiltered$time[match(rownames(Norm_unfiltered_taxonomy_g), Taxonomy_unfiltered$Sample)]
...
# assign time
Norm_log10_Module_abundance$time<-Taxonomy_unfiltered$time[match(rownames(Norm_log10_Module_abundance), Taxonomy_unfiltered$Sample)]
...
# ggplot(Module_rainbow, aes(module, Estimate,colour=module))+geom_line()+geom_errorbar(ymin=Module_rainbow$Estimate-Module_rainbow$`Std. Error`, ymax=Module_rainbow$Estimate+Module_rainbow$`Std. Error`)+geom_point(shape=21, size=1, fill="white")+theme_bw()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 6), legend.position="none")+geom_hline(aes(yintercept=0), colour="black", linetype="dashed")+ylim(-0.1,0.1)
...
# ggplot(Module_rainbow, aes(module, Estimate,colour=maxtime))+geom_line()+geom_errorbar(aes(ymin=Module_rainbow$Estimate-Module_rainbow$`Std. Error`, ymax=Module_rainbow$Estimate+Module_rainbow$`Std. Error`))+geom_point(shape=21, size=1, fill="white")+theme_bw()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 8), legend.position="top")+geom_hline(aes(yintercept=0), colour="black", linetype="dashed")+ylim(-0.1,0.1)+ scale_color_manual(values=c("#E69F00", "#56B4E9", "#FF4500","#551A8B"))
...
fig1B<-gg_ordiplot(mds, groups=meta0$caesar, scaling = 1, choices = c(1, 2), kind = "se", conf = 0.95, show.groups = "all", ellipse = TRUE, label = TRUE, hull = FALSE, spiders = FALSE, plot = TRUE, pt.size=1) 
...
fig1C<-gg_ordiplot(mds, groups=meta3$caesar, scaling = 1, choices = c(1, 2), kind = "se", conf = 0.95, show.groups = "all", ellipse = TRUE, label = TRUE, hull = FALSE, spiders = FALSE, plot = TRUE, pt.size=1)
...
fig1D<-gg_ordiplot(mds, groups=meta12$caesar, scaling = 1, choices = c(1, 2), kind = "se", conf = 0.95, show.groups = "all", ellipse = TRUE, label = TRUE, hull = FALSE, spiders = FALSE, plot = TRUE, pt.size=1)
...
fig1E<-gg_ordiplot(mds, groups=meta24$caesar, scaling = 1, choices = c(1, 2), kind = "se", conf = 0.95, show.groups = "all", ellipse = TRUE, label = TRUE, hull = FALSE, spiders = FALSE, plot = TRUE, pt.size=1)
